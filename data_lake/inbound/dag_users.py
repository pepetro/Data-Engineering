...
GCS_SOURCE_OBJECT_PATH = 'data/user_id_sample_data.csv'
GCS_OBJECT_PATH = 'data/user_id_sample_data_filtered.csv'
...
# 2) Remove records with null ids
def clean_ds():
    df = pd.read_csv('gs://' + GCS_BUCKET + '/' + GCS_SOURCE_OBJECT_PATH,
                     usecols=['user_id']).drop_duplicates(keep='first')
